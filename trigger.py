...
def analyze_triggers(df):
    ...
    # If 'Close' is a DataFrame (multi-level columns from yfinance), take first column
    ...
    if u_curve_formed:
        ...
        # Handle if crossed is a DataFrame (rare cases)
        ...
        if crossed.any():
            buy_trigger = True
            buy_date = crossed.idxmax()  # **FIXED**: direct index label (date)
        else:
            buy_trigger = False
            buy_date = None
